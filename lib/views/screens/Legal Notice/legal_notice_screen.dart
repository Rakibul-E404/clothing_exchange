...
class LegalNoticeData {
  ...
  LegalNoticeData({required this.content, required this.updatedAt});
}
...
class _LegalNoticeScreenState extends State<LegalNoticeScreen> {
  late Future<LegalNoticeData> _legalNoticeFuture;

  @override
  void initState() {
    super.initState();
    _legalNoticeFuture = fetchLegalNotice();
  }

  Future<LegalNoticeData> fetchLegalNotice() async {
    try {
      final token = SharedPrefHelper().getData(AppConstants.token);

      final response = await http.get(
        Uri.parse('${AppUrl.baseUrl}/legal'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        return LegalNoticeData(content: content, updatedAt: updatedAt);
      ...
      throw Exception('Failed to load legal notice: ${e.toString()}');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<LegalNoticeData>(
