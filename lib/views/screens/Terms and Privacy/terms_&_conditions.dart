...
class TermsAndConditionsData {
  ...
  TermsAndConditionsData({required this.content, required this.updatedAt});
}
...
class _TermsAndConditionsScreenState extends State<TermsAndConditionsScreen> {
  late Future<TermsAndConditionsData> _termsFuture;
  ...
  Future<TermsAndConditionsData> fetchTermsAndConditions() async {
    try {
      final token = SharedPrefHelper().getData(AppConstants.token);

      final response = await http.get(
        Uri.parse('${AppUrl.baseUrl}/term_condition'),
        headers: {
